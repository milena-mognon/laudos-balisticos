...
class BalinsChumboTest extends TestCase
{
    ...
    public function test_laudo_create_ok()
    {
        ...
        $componente = factory(Componente::class, 'Balins de Chumbo')->make([
            'laudo_id' => $this->laudo,
            '_token' => csrf_token()
        ]);

        $this->get(route('balins_chumbo.create', $this->laudo))->assertStatus(200);
    ...
    {
        $this->assertAuthenticatedAs($this->user);
        $componente = factory(Componente::class, 'Balins de Chumbo')->create([
            'laudo_id' => $this->laudo,
        ]);

        $this->followingRedirects()->get(route('componentes.edit', [$this->laudo, $componente]))->assertStatus(200)
            ->assertViewIs('perito.laudo.materiais.componentes.balins_chumbo.edit');

        $componente->toArray();
        $componente['quantidade'] = '8';
        $updated_data = factory(Componente::class, 'Balins de Chumbo')->make($componente->toArray());
    ...
    public function teste_componente_destroy_ok()
    {
        ...
        $this->get(route('laudos.show', $this->laudo))
            ...
            ->assertSeeText(strtoupper($componente->componente))
        ...
        $this->get(route('laudos.show', $this->laudo))
            ->assertStatus(200)
            ->assertViewIs('perito.laudo.show')
            ->assertDontSeeText(strtoupper($componente->componente))
