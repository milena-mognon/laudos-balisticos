...
class PolvoraTest extends TestCase
{
    ...
    public function test_laudo_create_ok()
    {
        ...
        $componente = factory(Componente::class, 'Polvora')->make([
            'laudo_id' => $this->laudo,
            '_token' => csrf_token()
        ]);

        $this->get(route('polvora.create', $this->laudo))->assertStatus(200);
    ...
    {
        $this->assertAuthenticatedAs($this->user);
        $componente = factory(Componente::class, 'Polvora')->create([
            'laudo_id' => $this->laudo,
        ]);

        $this->followingRedirects()->get(route('componentes.edit', [$this->laudo, $componente]))->assertStatus(200)
            ->assertViewIs('perito.laudo.materiais.componentes.polvora.edit');

        $componente->toArray();
        $componente['quantidade'] = '458';
        $updated_data = factory(Componente::class, 'Polvora')->make($componente->toArray());
