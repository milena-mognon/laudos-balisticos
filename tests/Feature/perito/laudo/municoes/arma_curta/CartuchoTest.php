...
namespace Tests\Feature\perito\laudo\municoes\arma_curta;
...
class CartuchoTest extends TestCase
{
    ...
    public function test_laudo_create_ok()
    {
        ...
        $cartucho = factory(Municao::class, 'Cartucho Arma Curta')->make([
            'laudo_id' => $this->laudo,
            '_token' => csrf_token()
        ]);

        $this->get(route('armas_curtas.create', $this->laudo))->assertStatus(200);
    ...
    {
        $this->assertAuthenticatedAs($this->user);
        $cartucho = factory(Municao::class, 'Cartucho Arma Curta')->create([
            'laudo_id' => $this->laudo,
        ]);

        $this->followingRedirects()->get(route('municoes.edit', [$this->laudo, $cartucho]))->assertStatus(200)
            ->assertViewIs('perito.laudo.materiais.municoes.arma_curta.edit');

        $cartucho->toArray();
        $cartucho['marca_id'] = factory(Marca::class);

        $updated_data = factory(Municao::class, 'Cartucho Arma Curta')
