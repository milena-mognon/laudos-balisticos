...
class EstojoTest extends TestCase
{
    ...
    public function test_laudo_create_ok()
    {
        ...
        $cartucho = factory(Municao::class, 'Estojo Arma Longa')->make([
            'laudo_id' => $this->laudo,
            '_token' => csrf_token()
        ]);
    ...
    {
        $this->assertAuthenticatedAs($this->user);
        $cartucho = factory(Municao::class, 'Estojo Arma Longa')->create([
            'laudo_id' => $this->laudo,
        ]);
        ...
        $updated_data = factory(Municao::class, 'Estojo Arma Longa')
